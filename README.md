...
judge の主要言語が壊れていないかを、`aplusb` 問題の先頭ケースでまとめて確認できます。

```bash
judge-smoke-test
```

このコマンドは dev shell 内で `cargo test smoke_aplusb_across_languages -- --nocapture`
を実行します。初回は依存クレートや各言語ツールチェーンのビルドで時間がかかります。
...
テストの中で実際にやっていることは、各言語ごとに最小の `A+B` プログラムをその場で生成し、
`compile()` と `run_in_sandbox()` を通して 1 ケースだけ実行することです。
つまり、少なくとも以下は通っていると判断できます。

- その言語のコンパイラまたはインタプリタが見つかる
- ジャッジ用のコンパイル手順が壊れていない
- サンドボックス内で実行できる
- 期待した出力になっている

一方で、これはあくまでスモークテストなので、以下までは保証しません。

- 長時間実行や大きいメモリ使用時の挙動
- 複数ケース実行時の安定性
- 問題ごとの特殊な入出力や大きいソースコード
- Linux 本番相当の厳密な計測値
...
失敗時は、その言語名つきで panic するようにしてあるので、
どの言語で `compile` または `run` が壊れたかをそのまま追えます。
...
提出ソースコードのサイズ上限は 512 KiB です。
上限を超える提出は `413 Payload Too Large` として拒否され、ジャッジキューには入りません。
...
提出一覧は `/admin/submissions` で確認できます。
